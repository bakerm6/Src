...
var float SettingsSliderStart, SettingsSliderEnd;
...
/**
 * Set up variables for sliding the Settings button
 */
function AutoSlideSettings(float Destination)
{
	SettingsSliderStart = PauseMenu.MenuObjects[4].Left;
	SettingsSliderEnd = Destination;
}
...
function ResetMenu()
{
	if (bPauseMenuOpen)
	{
		...
		AutoSlideSettings(PauseMenu.Width);
...
function OpenMenu()
{
	if (!bPauseMenuOpen)
	{
		...
		AutoSlideSettings(PauseMenu.Width - PauseMenu.MenuObjects[4].Width + 2);
...
function PlayerTick(float DeltaTime)
{
	...
	if (bAutoSlide)
	{
		...
		SettingsLocationX = FInterpEaseInOut(SettingsSliderStart, SettingsSliderEnd, SliderTravelTime/SliderTravelDuration, 3.0);
		SliderTravelTime += DeltaTime;
		if (SliderTravelTime >= SliderTravelDuration)
		{
			SliderZone.CurrentLocation.Y = SliderEnd;
			SettingsLocationX = SettingsSliderEnd;
			bAutoSlide = false;
